...
#pragma omp parallel
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int s = 0; s < 1U << i; s++)
            {
                for (int k = 0; k < N >> i; k++)
                {
                    int z = s * (N >> i) + k;
                    L[i][z] = decoder_t(std::vector<softdata_t>(dec_param.ns[i][s][1],1e6), std::vector<uint16_t>(dec_param.ns[i][s][1],q));
                    // cout<<dec_param.ns[i][s][1]<<" ";
        ...
        for (int k = 0; k < N; k++)
        {
            if(dec_param.ufrozen[n][k]!=0)
            L[n][k] = decoder_t(std::vector<softdata_t>(1,1e6), std::vector<uint16_t>(1,q));
            else
            L[n][k] = decoder_t(std::vector<softdata_t>(0), std::vector<uint16_t>(0));
        }
