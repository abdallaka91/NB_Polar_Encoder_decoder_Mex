...

{
    ...
    theta.intrinsic_GF.assign(nm, q);
    for (int i = 0; i < nm; i++)
    {
        theta.intrinsic_GF[i] = temp_GF[i];
        theta.intrinsic_LLR[i] = temp_llr[temp_GF[i]];
                                   ...
                                   decoder_t &phi)
{
    ...
    uint16_t q = dec_param.q;
    uint16_t nm = theta_1.intrinsic_GF.size();
    vector<softdata_t> theta1_llr(q, theta_1.intrinsic_LLR.back() + dec_param.offset);
    vector<softdata_t> phi1_llr(q, phi_1.intrinsic_LLR.back() + dec_param.offset);
    vector<softdata_t> temp_llr(q);
    for (int i = 0; i < phi_1.intrinsic_GF.size(); i++)
    {
        phi1_llr[phi_1.intrinsic_GF[i]] = phi_1.intrinsic_LLR[i];
    ...
    for (int i = 0; i < theta_1.intrinsic_GF.size(); i++)
    {
        theta1_llr[ADDGF[hard_decision][theta_1.intrinsic_GF[i]]] = theta_1.intrinsic_LLR[i];
    }
    ...
    {
        for (int i = 1; i < phi.intrinsic_GF.size(); i++)
        {
            std::cout << phi.intrinsic_LLR[i] << " ";
        }
        std::cout << std::endl;
    }
}
    ...
    while (l > -1)
    {
        ...
        else if (Roots[l + 1][2 * s])
        {
            ...
            for (uint16_t t = 0; t < SZc1; t++)
            {
                ...
                if (dec_param.ufrozen[l + 1][Root[t] + SZc1] != dec_param.frozen_val)
                {
